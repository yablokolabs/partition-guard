...
void test_timing_reasonable() {
    ...
    assert(elapsed_ms >= 250 && elapsed_ms <= 400 &&
           "10 frames at 30ms should take ~300ms");
...
void test_statistics_tracking() {
    ...
    for (std::uint8_t i = 0; i < 3; ++i) {
        if (sched.partition(i).iteration != 5) return std::fprintf(stderr, "  ✗ iteration\n"), (void)0;
        if (sched.partition(i).last_exec_ns <= 0) return std::fprintf(stderr, "  ✗ last_exec\n"), (void)0;
        if (sched.partition(i).worst_exec_ns <= 0) return std::fprintf(stderr, "  ✗ worst_exec\n"), (void)0;
        if (sched.partition(i).total_exec_ns <= 0) return std::fprintf(stderr, "  ✗ total_exec\n"), (void)0;
    }
